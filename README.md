...
- `nushell-completion` file from <https://github.com/kerl/kerl/raw/master/nushell_completion/kerl.nu>
...
```console
$ kerl emit-activate <release> <build_name> <directory> [sh|bash|fish|csh|nushell]
```
...
`activate.fish`, `activate.csh`, or `activate.nu`, set the last argument to `fish`, `csh`, or `nushell`, respectively.
...
```console
$ overlay use /path/to/install/dir/activate.nu
```
